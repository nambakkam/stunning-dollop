...
void VectorImplementation::setVectorSize(sizeEnum value)
{
    ...
    QFuture<void> vectorFuture = QtConcurrent::run(this,&VectorImplementation::generateRandomVector,size);
...
{
    int randomNumber = RandomValuesGenerator::getInstance().generateRandomInt(LOWERLIMIT,UPPERLIMIT);
    vector.push_back(randomNumber);
    set_description(QString("Random number %1 pushed to the vector").arg(randomNumber));
    emit vectorChanged();
...
void VectorImplementation::popLastNumber()
{
    ...
    vector.pop_back();
    set_description(QString("Last Number %1 popped from the vector").arg(lastNumber));
    emit vectorChanged();
}

void VectorImplementation::insertatRandomIndex()
{
    int randomIndex = RandomValuesGenerator::getInstance().generateRandomInt(0,vector.size());
    int randomNumber = RandomValuesGenerator::getInstance().generateRandomInt(LOWERLIMIT,UPPERLIMIT);
    vector.insert(randomIndex,randomNumber);
    set_description(QString("%1 inserted at the index %2").arg(randomNumber).arg(randomIndex));
    emit vectorChanged();
}
...
void VectorImplementation::searchRandomNumber()
{
    ...
    if(numberPresent){
        ...
        set_description(QString("%1 is not present in the vector ").arg(randomNumber));
    }
}
...
void VectorImplementation::valueAtRandomIndex()
{
        ...
        set_description(QString("%1 is found at the index %2 of the vector ").arg(number).arg(randomIndex));
...
void VectorImplementation::removeatRandomIndex()
{
    ...
    set_description(QString("Value %1 Removed at the index ").arg(randomValue) + QString::number(randomIndex));
    emit vectorChanged();
...
void VectorImplementation::generateRandomVector(size_t size)
{
    ...
    for(size_t i=0; i< size; i++){
        vector.push_back(RandomValuesGenerator::getInstance().generateRandomInt(LOWERLIMIT,UPPERLIMIT));
    }
    qDebug() << "First 3 elements " << vector.at(0) << "," << vector.at(1) << "," << vector.at(2);
}
